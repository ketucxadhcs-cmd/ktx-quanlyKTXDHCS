...
// Hai mã khôi phục TÁCH RIÊNG theo cấp quyền — Cán bộ quản lý giữ mã "staff" chỉ tự đặt lại được mật khẩu
// Học viên / Quản lý ký túc xá; mã "admin" (quyền cao nhất) chỉ Quản trị viên biết, và chỉ mã này mới đặt lại
// được mật khẩu Quản trị. Ai biết mã staff KHÔNG thể tự nâng quyền lên Quản trị bằng con đường Quên mật khẩu.
const STAFF_RECOVERY_CODE_DEFAULT = "KTXKHOIPHUCCB01";
const ADMIN_RECOVERY_CODE_DEFAULT = "KTXKHOIPHUCQT01";
const DATA_NS = "ktxcsnd"; // Tên collection Firestore riêng cho trang Ký túc xá (không lẫn dữ liệu khác)

// ===== Cấu hình gửi mã OTP qua email cho Quản trị viên khi quên cả Mã khôi phục Quản trị =====
// Dùng dịch vụ EmailJS (miễn phí, gửi email thẳng từ trình duyệt, không cần máy chủ riêng).
...
function useAuthConfig() {
  const [config, setConfigState] = useState({
    unitPassword: KTX_PASSWORD_DEFAULT,
    managerPassword: MANAGER_PASSWORD_DEFAULT,
    adminPassword: ADMIN_PASSWORD_DEFAULT,
    staffRecoveryCode: STAFF_RECOVERY_CODE_DEFAULT,
        ...
        if (snap.exists() && snap.data().value) {
          try {
            ...
            setConfigState({
              ...
              staffRecoveryCode: parsed.staffRecoveryCode || STAFF_RECOVERY_CODE_DEFAULT,
...
/* ============ LOGIN GATE ============ */
// Khung thẻ dùng chung cho màn hình đăng nhập và các bước "Quên mật khẩu" — giữ đồng nhất logo/tiêu đề.
function AuthCardShell({ subtitle, children }) {
  return (
      ...
      >
        ...
        <div className="relative px-8 pt-8 pb-8">
          ...
          <div className="relative flex flex-col items-center mb-6">
            ...
            <div className="f-body text-xs" style={{ color: T.inkSoft }}>{subtitle}</div>
          </div>
...
function BackLink({ onClick, children }) {
  return (
    <p className="f-body text-xs mt-3 text-center">
      <button type="button" onClick={onClick} className="underline" style={{ color: T.inkSoft }}>{children}</button>
    </p>
  );
}

// Bước 1 của "Quên mật khẩu": chọn cấp quyền, vì mỗi cấp chỉ tự đặt lại được đúng phạm vi của mình —
// tránh trường hợp Cán bộ quản lý biết mã của mình lại tự nâng quyền lên Quản trị được.
function ForgotChooseStep({ onChooseStaff, onChooseAdmin, onCancel }) {
  return (
    <div>
      <p className="f-body text-xs mb-4" style={{ color: T.inkSoft }}>Bạn đang quên mật khẩu ở vai trò nào?</p>
      <button
        type="button"
        onClick={onChooseStaff}
        className="w-full text-left px-3.5 py-3 mb-2.5 f-body text-sm btn-press"
        style={{ border: `1px solid ${T.paperDark}`, background: T.paper }}
      >
        <div className="font-medium" style={{ color: T.ink }}>Cán bộ quản lý ký túc xá</div>
        <div className="text-[11px] mt-0.5" style={{ color: T.inkSoft }}>Tự đặt lại mật khẩu Học viên và/hoặc mật khẩu Quản lý ký túc xá</div>
      </button>
      <button
        type="button"
        onClick={onChooseAdmin}
        className="w-full text-left px-3.5 py-3 mb-3 f-body text-sm btn-press"
        style={{ border: `1px solid ${T.paperDark}`, background: T.paper }}
      >
        <div className="font-medium" style={{ color: T.ink }}>Quản trị viên</div>
        <div className="text-[11px] mt-0.5" style={{ color: T.inkSoft }}>Đặt lại mật khẩu Quản trị, hoặc cả 3 mật khẩu</div>
      </button>
      <BackLink onClick={onCancel}>← Quay lại đăng nhập</BackLink>
    </div>
  );
}

// Cán bộ quản lý quên mật khẩu: chỉ cần Mã khôi phục "staff" — chỉ động được tới mật khẩu Học viên và/hoặc
// mật khẩu Quản lý ký túc xá, KHÔNG thể chạm tới mật khẩu Quản trị bằng con đường này.
function ForgotStaffPanel({ config, setConfig, onBack, onDone }) {
  const [code, setCode] = useState("");
  const [resetUnit, setResetUnit] = useState(true);
  const [resetManager, setResetManager] = useState(true);
  ...
  const submit = async (e) => {
    ...
    if (resetUnit && !newUnitPw.trim()) { setErr("Vui lòng nhập mật khẩu chung ký túc xá mới."); return; }
    if (resetManager && !newManagerPw.trim()) { setErr("Vui lòng nhập mật khẩu quản lý ký túc xá mới."); return; }
    setSaving(true);
    const ok = await setConfig({
      ...config,
    ...
    setMsg("Đã đặt lại mật khẩu thành công. Bạn có thể đăng nhập ngay bằng mật khẩu mới.");
    ...
    <form onSubmit={submit}>
      ...
      <div className="mb-3">
        <label className="flex items-center gap-2 f-body text-xs mb-2" style={{ color: T.ink }}>
          <input type="checkbox" checked={resetUnit} onChange={(e) => setResetUnit(e.target.checked)} />
          Đặt lại mật khẩu chung ký túc xá (Học viên)
        </label>
        {resetUnit && (
        ...
        <label className="flex items-center gap-2 f-body text-xs mb-2" style={{ color: T.ink }}>
          <input type="checkbox" checked={resetManager} onChange={(e) => setResetManager(e.target.checked)} />
          Đặt lại mật khẩu quản lý ký túc xá
        </label>
        {resetManager && (
      ...
      {err && (
        <div className="f-body text-xs mb-3 px-3 py-2 flex items-start gap-2" style={{ color: T.red, background: "#F7E3E6", borderLeft: `3px solid ${T.red}` }}>{err}</div>
      )}
      {msg && (
        <div className="f-body text-xs mb-3 px-3 py-2" style={{ color: T.green, background: "#E5F0E7", borderLeft: `3px solid ${T.green}` }}>{msg}</div>
      )}
...
// Quản trị quên mật khẩu: dùng Mã khôi phục "admin" (khác hoàn toàn mã của Cán bộ), hoặc nếu quên luôn mã này
// thì bấm "Gửi mã OTP qua email" để nhận mã dùng 1 lần gửi tới Gmail quản trị đã đăng ký sẵn trong code.
// Được chọn đổi TẤT CẢ 3 mật khẩu, hoặc CHỈ đổi riêng mật khẩu Quản trị.
function ForgotAdminPanel({ config, setConfig, onBack, onDone }) {
  const [code, setCode] = useState("");
  const [scope, setScope] = useState("adminOnly"); // "adminOnly" | "all"
  ...
  const sendOtp = async () => {
    ...
    // Lưu OTP vào Firestore trước, để verify được kể cả khi gửi email xong người dùng mở máy khác nhập mã.
    const saved = await setConfig({ ...config, otpCode: otp, otpExpiresAt: expiresAt });
    if (!saved) { setOtpSending(false); setOtpStatus("Không lưu được mã OTP, thử lại nhé."); return; }
    try {
  ...
  const submit = async (e) => {
    ...
    setErr(""); setMsg("");
    const codeTrim = code.trim();
    if (!codeTrim) { setErr("Vui lòng nhập Mã khôi phục Quản trị hoặc mã OTP đã nhận qua email."); return; }
    const matchesRecoveryCode = codeTrim === config.adminRecoveryCode;
    const matchesOtp = config.otpCode && codeTrim === config.otpCode && Date.now() < (config.otpExpiresAt || 0);
    if (!matchesRecoveryCode && !matchesOtp) {
      setErr("Mã không đúng hoặc mã OTP đã hết hạn. Kiểm tra lại, hoặc bấm \"Gửi mã OTP qua email\" để nhận mã mới.");
      return;
    }
    if (!newAdminPw.trim()) { setErr("Vui lòng nhập mật khẩu Quản trị mới."); return; }
    if (scope === "all" && (!newUnitPw.trim() || !newManagerPw.trim())) {
      setErr("Bạn chọn đổi cả 3 mật khẩu — vui lòng nhập đủ mật khẩu Học viên và Quản lý ký túc xá mới.");
      return;
    }
    setSaving(true);
    const ok = await setConfig({
      ...
      unitPassword: scope === "all" ? newUnitPw.trim() : config.unitPassword,
      managerPassword: scope === "all" ? newManagerPw.trim() : config.managerPassword,
      otpCode: "", // huỷ OTP ngay sau khi dùng, tránh dùng lại
      otpExpiresAt: 0,
    });
    setSaving(false);
    if (!ok) { setErr("Lưu thất bại, thử lại nhé."); return; }
  ...
  return (
    <form onSubmit={submit}>
      <p className="f-body text-xs mb-4" style={{ color: T.inkSoft }}>
        Nhập <b>Mã khôi phục Quản trị</b> (khác mã của Cán bộ quản lý). Nếu không nhớ mã này, bấm "Gửi mã OTP qua email" để
        nhận mã dùng 1 lần gửi tới Gmail quản trị đã đăng ký sẵn.
      </p>
      <Field label="Mã khôi phục Quản trị hoặc mã OTP" required>
        <PasswordInput value={code} onChange={(e) => setCode(e.target.value)} placeholder="Nhập mã khôi phục hoặc mã OTP 6 số" />
      </Field>
      <button
        type="button"
        ...
        style={{ color: T.amberDark }}
      >
        {otpSending ? "Đang gửi…" : `Gửi mã OTP qua email (${ADMIN_RECOVERY_EMAIL})`}
      </button>
      {otpStatus && <div className="f-body text-[11px] mb-3" style={{ color: T.inkSoft }}>{otpStatus}</div>}

      <div className="mb-3">
        <div className="f-body text-xs mb-1.5" style={{ color: T.ink }}>Phạm vi đặt lại</div>
        <label className="flex items-center gap-2 f-body text-xs mb-1.5" style={{ color: T.ink }}>
          <input type="radio" name="scope" checked={scope === "adminOnly"} onChange={() => setScope("adminOnly")} />
          Chỉ đặt lại mật khẩu Quản trị
        </label>
        <label className="flex items-center gap-2 f-body text-xs mb-2" style={{ color: T.ink }}>
          <input type="radio" name="scope" checked={scope === "all"} onChange={() => setScope("all")} />
          Đặt lại cả 3 mật khẩu (Học viên, Quản lý ký túc xá, Quản trị)
        </label>
      </div>
...
function LoginGate({ onLogin }) {
  ...
  const [forgotStep, setForgotStep] = useState(null); // null | "choose" | "staff" | "admin"
  ...
  if (forgotStep === "choose") {
    return (
      <AuthCardShell subtitle="Quên mật khẩu — chọn vai trò">
        <ForgotChooseStep
          onChooseStaff={() => setForgotStep("staff")}
          onChooseAdmin={() => setForgotStep("admin")}
          onCancel={() => setForgotStep(null)}
        />
      </AuthCardShell>
    );
  }
  if (forgotStep === "staff") {
    return (
      <AuthCardShell subtitle="Quên mật khẩu — Cán bộ quản lý">
        <ForgotStaffPanel config={config} setConfig={setConfig} onBack={() => setForgotStep("choose")} onDone={() => setForgotStep(null)} />
      </AuthCardShell>
    );
  }
  if (forgotStep === "admin") {
    return (
      <AuthCardShell subtitle="Quên mật khẩu — Quản trị viên">
        <ForgotAdminPanel config={config} setConfig={setConfig} onBack={() => setForgotStep("choose")} onDone={() => setForgotStep(null)} />
      </AuthCardShell>
    );
  }

  return (
    <AuthCardShell subtitle="Cổng truy cập nội bộ ký túc xá">
      <form onSubmit={submit}>
        ...
        <BackLink onClick={() => setForgotStep("choose")}>Quên mật khẩu?</BackLink>
      </form>
    </AuthCardShell>
  );
}
...
let _emailjsLoadPromise = null;
function loadEmailJSLib() {
  if (window.emailjs) return Promise.resolve(window.emailjs);
  if (_emailjsLoadPromise) return _emailjsLoadPromise;
  _emailjsLoadPromise = new Promise((resolve, reject) => {
    const script = document.createElement("script");
    script.src = "https://cdn.jsdelivr.net/npm/@emailjs/browser@4/dist/email.min.js";
    script.onload = () => resolve(window.emailjs);
    script.onerror = () => reject(new Error("Không tải được thư viện gửi email — kiểm tra kết nối mạng."));
    document.head.appendChild(script);
  });
  return _emailjsLoadPromise;
}
// Gửi mã OTP 6 số về Gmail của Quản trị (ketucxadhcs@gmail.com) khi Quản trị quên cả Mã khôi phục.
// Cần cấu hình EMAILJS_SERVICE_ID / EMAILJS_TEMPLATE_ID / EMAILJS_PUBLIC_KEY ở đầu file (xem hướng dẫn phía trên).
async function sendRecoveryOtpEmail(otp) {
  if (EMAILJS_SERVICE_ID.startsWith("PASTE_") || EMAILJS_TEMPLATE_ID.startsWith("PASTE_") || EMAILJS_PUBLIC_KEY.startsWith("PASTE_")) {
    throw new Error("Chưa cấu hình dịch vụ gửi email (EmailJS) trong code — xem chú thích EMAILJS_SERVICE_ID ở đầu file App.jsx.");
  }
  const emailjs = await loadEmailJSLib();
  emailjs.init(EMAILJS_PUBLIC_KEY);
  await emailjs.send(EMAILJS_SERVICE_ID, EMAILJS_TEMPLATE_ID, {
    to_email: ADMIN_RECOVERY_EMAIL,
    otp_code: otp,
    valid_minutes: String(OTP_TTL_MS / 60000),
  });
}
...
function PasswordTab({ user, perm }) {
  ...
  const [staffRecoveryCode, setStaffRecoveryCode] = useState("");
  ...
  useEffect(() => {
    ...
    setStaffRecoveryCode(config.staffRecoveryCode);
    setAdminRecoveryCode(config.adminRecoveryCode);
  }, [config.unitPassword, config.managerPassword, config.adminPassword, config.staffRecoveryCode, config.adminRecoveryCode]);

  // Cán bộ quản lý (can_bo) chỉ được xem/đổi mật khẩu Học viên và mật khẩu Quản lý ký túc xá của chính mình —
  // KHÔNG được xem hay đổi mật khẩu Quản trị, cũng không thấy 2 Mã khôi phục. Chỉ tài khoản Quản trị mới toàn quyền.
  const saveAdmin = async () => {
    if (!unitPw.trim() || !managerPw.trim() || (perm.isAdmin && (!adminPw.trim() || !staffRecoveryCode.trim() || !adminRecoveryCode.trim()))) {
      setWarn(
        perm.isAdmin
          ? "Vui lòng nhập đủ cả 3 mật khẩu và 2 Mã khôi phục trước khi lưu."
          : "Vui lòng nhập đủ Mật khẩu chung ký túc xá và Mật khẩu quản lý ký túc xá trước khi lưu."
      );
      return;
    }
    if (perm.isAdmin && staffRecoveryCode.trim() === adminRecoveryCode.trim()) {
      setWarn("Mã khôi phục Cán bộ và Mã khôi phục Quản trị phải KHÁC NHAU — nếu trùng, ai biết mã cũng chiếm được quyền Quản trị.");
      return;
    }
    setWarn("");
    setSaving(true);
    const ok = await setConfig({
      ...
      staffRecoveryCode: perm.isAdmin ? staffRecoveryCode.trim() : config.staffRecoveryCode,
      adminRecoveryCode: perm.isAdmin ? adminRecoveryCode.trim() : config.adminRecoveryCode,
      otpCode: config.otpCode,
      otpExpiresAt: config.otpExpiresAt,
    });
    setSaving(false);
        ...
        <div className="stamp-border p-4" style={{ background: "#fff" }}>
          ...
          {perm.isAdmin && (
            <>
              ...
              <div className="mt-2 mb-1 pt-3" style={{ borderTop: `1px dashed ${T.paperDark}` }}>
                <div className="f-display text-xs uppercase tracking-wider mb-1.5" style={{ color: T.amberDark }}>
                  2 mã khôi phục — dùng ở màn hình đăng nhập khi có người quên mật khẩu
                </div>
                <p className="f-body text-[11px] mb-3" style={{ color: T.inkSoft }}>
                  Tách riêng theo cấp quyền để không ai tự nâng quyền lên Quản trị được: Cán bộ quản lý chỉ tự đặt lại được mật khẩu
                  Học viên / Quản lý ký túc xá bằng mã bên dưới đầu tiên; còn mật khẩu Quản trị chỉ đặt lại được bằng mã thứ hai
                  (do chính bạn giữ riêng, tuyệt đối không chia sẻ). Hai mã phải khác nhau.
                </p>
              </div>
              <Field label="Mã khôi phục dành cho Cán bộ quản lý (chỉ đổi được MK Học viên / Quản lý ký túc xá)" required>
                <PasswordInput value={staffRecoveryCode} onChange={(e) => setStaffRecoveryCode(e.target.value)} />
              </Field>
              <Field label="Mã khôi phục dành cho Quản trị (đổi được MK Quản trị, hoặc cả 3 mật khẩu) — giữ kín, chỉ mình bạn biết" required>
                <PasswordInput value={adminRecoveryCode} onChange={(e) => setAdminRecoveryCode(e.target.value)} />
              </Field>
              <p className="f-body text-[11px] mb-4 -mt-2" style={{ color: T.inkSoft }}>
                Nếu quên luôn cả mã này, màn hình đăng nhập có lựa chọn "Gửi mã OTP qua email" gửi về{" "}
                <b style={{ color: T.ink }}>{ADMIN_RECOVERY_EMAIL}</b> để dùng thay — cần cấu hình sẵn dịch vụ gửi email (EmailJS) trong code.
              </p>
            </>
          )}
